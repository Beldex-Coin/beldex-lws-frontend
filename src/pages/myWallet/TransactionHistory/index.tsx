...
export default function TransactionHistory() {
  ...
  const exportTransactionsCSV = () => {
    ...
    transactionHistory.forEach(function (tx: any, i: any) {
      const received_JSBigInt = tx.total_received
        ? typeof tx.total_received === "string"
          ? new JSBigInt(tx.total_received)
          : tx.total_received
        : new JSBigInt("0");
      const sent_JSBigInt = tx.total_sent
        ? typeof tx.total_sent === "string"
          ? new JSBigInt(tx.total_sent)
          : tx.total_sent
        : new JSBigInt("0");
      const amountString = beldex_amount_format_utils.formatMoney(
        received_JSBigInt.subtract(sent_JSBigInt)
      );
      const payment_id = `${tx.payment_id || ""}`;
      const status = `${
        tx.isFailed
          ? "REJECTED"
          : tx.isConfirmed !== true || tx.isUnlocked !== true
          ? "PENDING"
          : "CONFIRMED"
      }`;
      const columns = [
        ...
        const requestHandle =
          coreBridgeInstance.hostedMoneroAPIClient.AddressTransactions_returningRequestHandle(
            ...
            function (
              ...
              pooltxnValidation(customizeTxn);
              // let newTransaction = walletDetails.transactionPoolHistory;
              if (
  ...
  const pooltxnValidation = (txn: any) => {
    let valid = txn.filter(
      (item: any) => item.hash === walletDetails?.transactionPoolHistory?.hash
    );
    console.log("valid::", valid);
    if (walletDetails?.transactionPoolHistory?.hash && valid.length > 0) {
      console.log("pooltxnValidation");
